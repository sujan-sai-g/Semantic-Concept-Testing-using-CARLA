...
void export_sensor_data() {
  ...
  ;
    ...
    class_ < csd::PedDetectEvent,                    // actual type.
            bases < cs::SensorData >,                     // parent type.
            boost::noncopyable,                        // disable copy.
            boost::shared_ptr < csd::PedDetectEvent >  // use as shared_ptr.
            > ("PedDetectEvent", no_init)              // name, and disable construction.
                    .def("__len__", &csd::PedDetectEvent::size)
                    .def("__iter__", iterator<csd::PedDetectEvent>())
                    .def("__getitem__", +[](const csd::PedDetectEvent &self, size_t pos) -> cr::ActorId {
                        return self.at(pos);
                    });
}
