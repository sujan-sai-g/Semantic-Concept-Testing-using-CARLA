...
FActorDefinition UActorBlueprintFunctionLibrary::MakePedDetectorDefinition()
{
    FActorDefinition Definition;
    bool Success;
    MakePedDetectorDefinition(Success, Definition);
    check(Success);
    return Definition;
}

void UActorBlueprintFunctionLibrary::MakePedDetectorDefinition(
        bool &Success,
        FActorDefinition &Definition)
{
    FillIdAndTags(Definition, TEXT("sensor"), TEXT("other"), TEXT("ped_detector"));
    AddVariationsForSensor(Definition);
    ...
    FActorVariation UpperYDistance;
    UpperYDistance.Id = TEXT("upper_y_distance");
    UpperYDistance.Type = EActorAttributeType::Int;
    UpperYDistance.RecommendedValues = { TEXT("400") };
    UpperYDistance.bRestrictToRecommended = false;

    FActorVariation LowerYDistance;
    LowerYDistance.Id = TEXT("lower_y_distance");
    LowerYDistance.Type = EActorAttributeType::Int;
    LowerYDistance.RecommendedValues = { TEXT("4000") };
    LowerYDistance.bRestrictToRecommended = false;
...
void UActorBlueprintFunctionLibrary::SetPedDetector(
        const FActorDescription &Description,
        APedDetectionSensor *PedDetectionSensor)
{
    CARLA_ABFL_CHECK_ACTOR(PedDetectionSensor);
    constexpr float TO_CENTIMETERS = 1e2;

    PedDetectionSensor->SetHorizontalFOV(
            RetrieveActorAttributeToFloat("horizontal_fov", Description.Variations, 30.0f));
    PedDetectionSensor->SetVerticalFOV(
            RetrieveActorAttributeToFloat("vertical_fov", Description.Variations, 30.0f));
    PedDetectionSensor->SetRange(
            RetrieveActorAttributeToFloat("range", Description.Variations, 100.0f) * TO_CENTIMETERS);
    PedDetectionSensor->SetPointsPerSecond(
            RetrieveActorAttributeToInt("points_per_second", Description.Variations, 1500));
    PedDetectionSensor->SetLowerYDistance(
            RetrieveActorAttributeToInt("lower_y_distance", Description.Variations, 400));
    PedDetectionSensor->SetUpperYDistance(
            RetrieveActorAttributeToInt("upper_y_distance", Description.Variations, 4000));
}
